...
## Model 1.0 (*M~1.0~*): CART DT Using `rpart()`
```{r, fig.height=8, fig.width=10}
train_xy01_df01 <- data.frame(train_x01_df01, train_y01_df01)

# Train model
set.seed(100)
m1v0_cart <- rpart(x ~ ., data = train_xy01_df01, method = "class")
rpart.plot(m1v0_cart, type = 5, extra = 104)

# Use trained model to predict y on train data
m1v0_cart_y_pred_df01 <- predict(object = m1v0_cart, newdata = train_x01_df01, type = "class")

# Confirm confusion matrix results
confusionMatrix(table(m1v0_cart_y_pred_df01, train_y01_df01$x), positive = "RB")

# Use trained model to predict y on test data
test_m1v0_cart_y_pred_df01 <- predict(object = m1v0_cart, newdata = test_x01_df01, type = "class")

# Confirm confusion matrix results
confusionMatrix(table(test_m1v0_cart_y_pred_df01, test_y01_df01$x), positive = "RB")
```
...
twoClassSummary(m1v1_cart$pred, lev = c("NRB", "RB"))
...
confusionMatrix(test_pred_model_outcomes$M1.2.C5.0, test_pred_model_outcomes$obs, positive = "RB")
plot(m1v1_cart)
```
...
write.csv(test_pred_model_outcomes,
          "../data/01_biodeg_test_outcomes.csv", row.names = FALSE)
write.csv(data.frame(models_compare_summ$models, models_compare_summ$statistics),
          "../data/01_biodeg_test_resamp_results.csv", row.names = FALSE)
```
