...
m5v2_glr <- train(x = train_x01_df01, y = train_y01_vc01,
                  method = "glm",
                  preProcess = c("nzv", "corr", "BoxCox", "center", "scale", "spatialSign", "pca"),
                  metric = "ROC",
                  trControl = train_y01_vc01_2cl_ctl)
...
## Model 5.3 (*M~5.3~*): General Logistic Regression (GLR) w/ Feature Selection
```{r, fig.height=5, fig.width=10, warning=FALSE}
...
test_pred_model_outcomes$M5.3.GLR_Selected <- predict(m5v3_glr, test_x02_df01)
```
...
## Model 6.2 (*M~6.2~*): Penalized Logistic Regression (PLR) w/ Feature Selection
```{r, fig.height=5, fig.width=10}
# Lambda controls regularization; alpha regulates region: 1 = lasso, 0 = ridge, 0<a<1 = combo
set.seed(100)
m6v2_plr <- train(x = train_x02_df01, y = train_y01_vc01,
                  ...
                  trControl = train_y01_vc01_2cl_ctl,
                  tuneGrid = m6v1_plr_grd)
test_pred_model_outcomes$M6.2.PLR_Selected <- predict(m6v2_plr, test_x02_df01)
```
...
models_compare <- resamples(list(M5.1.GLR = m5v1_glr,
                                 ...
                                 M5.3.GLR_Selected = m5v3_glr,
                                 M6.1.PLR = m6v1_plr,
                                 M6.2.PLR_Selected = m6v2_plr))
...
legend("bottomright", legend=c("GLR", "GLR-PCA", "GLR-Selected", "PLR", "PLR-Selected"),
...
print(paste0("Model 5.3: GLR-Selected AUC = ", round(m5v3_glr_roc$auc, sig)))
print(paste0("Model 6.1: PLR AUC = ", round(m6v1_plr_roc$auc, sig)))
print(paste0("Model 6.2: PLR-Selected AUC = ", round(m6v2_plr_roc$auc, sig)))
...
confusionMatrix(test_pred_model_outcomes$M5.3.GLR_Selected, as.factor(test_pred_model_outcomes$obs), positive = "RB")
confusionMatrix(test_pred_model_outcomes$M6.1.PLR, as.factor(test_pred_model_outcomes$obs), positive = "RB")
confusionMatrix(test_pred_model_outcomes$M6.2.PLR_Selected, as.factor(test_pred_model_outcomes$obs), positive = "RB")
```

```{r}
write.csv(test_pred_model_outcomes,
          "../data/test-results/y_pred_amc.csv", row.names = FALSE)
write.csv(data.frame(models_compare_summ$models, models_compare_summ$statistics),
          "../data/05-06_biodeg_test_resamp_results.csv", row.names = FALSE)
```
